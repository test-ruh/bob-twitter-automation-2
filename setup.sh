...
phase_env() {
  ...
  "TWITTER_X_API_CREDENTIAL|Twitter/X API credential|Twitter/X account credential with permission to create the daily public post.|Open https://developer.x.com in your browser\nSign in with the Twitter/X account that should publish the greeting\nOpen the developer portal or project that has posting access\nCreate or select app credentials with permission to create posts\nCopy the credential value and paste it into the field above."
  )
...
phase_db() {
  ...
  if [[ -n "${PG_CONNECTION_STRING:-}" ]]; then
    ...
    if ${local_pg}; then
      ...
      warn "     Then set in .env: PG_CONNECTION_STRING=postgresql://postgres:<choose-a-password>@localhost:5432/bob"
      exit 1
    fi
...
phase_finish() {
  ...
  if command -v openclaw >/dev/null 2>&1; then
    if openclaw cron list 2>/dev/null | grep -q "^daily-morning-greeting\b"; then
      ok "cron 'daily-morning-greeting' already registered"
    else
      openclaw cron add --file cron/daily-morning-greeting.json && ok "registered cron 'daily-morning-greeting'"
    fi
  else
    warn "openclaw CLI not found — register manually: openclaw cron add --file cron/daily-morning-greeting.json"
  fi
  ...
  echo ""
  log "Useful commands:"
  log "  openclaw cron list                         # confirm scheduled job is registered"
  log "  openclaw cron runs                         # review run history without triggering a post"
  log "  python3 scripts/data_writer.py query --table result_scheduled_runs --limit 10"
}
